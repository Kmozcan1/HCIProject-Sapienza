...
private const val COMPANY_UPDATED_MESSAGE = "You have updated a company successfully!"
...
class InsertCompanyActivity : AppCompatActivity() {
    ...
    private fun submitCompany() {
        if (intent.getStringExtra("from") == "companyList") {
            when(submissionValid()) {
                true -> {
                    Utilities.updateCompany(createCompany())
                    Utilities.handleProgressBarAction(insertCompany_progressBar, window, false)
                    Toast.makeText(applicationContext,
                            COMPANY_UPDATED_MESSAGE, Toast.LENGTH_LONG).show()
                }
                false -> {
                    Utilities.handleProgressBarAction(insertCompany_progressBar, window, false)
                }
    ...
    private fun submissionValid(): Boolean {
        ...
        for (company in companyList!!) {
            ...
            && intent.getStringExtra("from") != "companyList") {
                Toast.makeText(applicationContext,
    ...
    private fun handleIncomingIntent(intent: Intent) {
        if (intent.getStringExtra("from") == "companyList") {
            if (intent.getStringExtra("companyKey") != null) {
                setTitle(R.string.editCompany)
                Utilities.getSingleCompany(intent.getStringExtra("companyKey"), object : FireBaseListener {
                    override fun onCallBack(value: Any, listener: ValueEventListener, query: DatabaseReference) {
                        val company = value as Company
                        companyName_autoCompleteTextView.setText(company.companyName)
                        openTime_timePicker.hour = company.openTime.take(2).toInt()
                        openTime_timePicker.minute = company.openTime.takeLast(2).toInt()
                        closeTime_timePicker.hour = company.closeTime.take(2).toInt()
                        closeTime_timePicker.minute = company.closeTime.takeLast(2).toInt()
                        companyImage_imageView.setImageBitmap(imageTransform.StringToBitmap(company.image))
                    }
                })
            }
        }
